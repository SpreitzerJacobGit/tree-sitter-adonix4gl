...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_adonix4gl", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_adonix4gl": ["*.pyi", "py.typed"],
        "tree_sitter_adonix4gl.queries": ["*.scm"],
    },
    ext_package="tree_sitter_adonix4gl",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_adonix4gl/binding.c",
                "src/parser.c",
            ],
            define_macros=[
